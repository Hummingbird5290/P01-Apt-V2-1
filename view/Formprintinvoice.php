...
<?php  echo "<br><div class='col-md-12'><div class='alert callout callout-info'>
        ...
        <h4><i class='icon fa fa-ban'></i> รายการพิมพ์ใบแจ้งหนี้ !!!</h4>
        คือการออกบิลแจ้งหนี้ให้แต่ละห้อง </div></div><br>";
?>
  <div class="col-md-12">
    <div class="box">
     ...
     $Configtype = new sumconfigallHm();
      if(!isset($_SESSION)){
       ...
       if (isset($_GET['id']) OR isset($_GET['mode'])) {
         ...
         if($_REQUEST['mode'] == 'update7'){
         $result = $Configtype->UpdateFlagStatus7($idbill);
         if($result){
           echo "<div class='col-md-2'></div><div class='col-md-8'>
           <div class='alert callout callout-success'>
           <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
           <h4><i class='icon fa fa-ban'></i> ยืนยันสำเร็จ!</h4>ยืนยันพิมพ์ใบแจ้งหนี้สำเร็จ
