  <?php
                ...
                LEFT JOIN booking b ON b.id= room.Book_Id 
                Where ( room.Status_Room <> 1 ) 
                and   (b.Status_Book = 0 and b.delete_by is null)
                ";
                $sqlquery = $sql;
                ...
                while( $row=mysqli_fetch_array($query) ) {  // preparing an array
                                ...
                                if($row["Status_Room"]==2 or $row["Status_Room"]==4)
                                {
                                        $Flag = "Save";
                                        if(!isset($Book_Id))
                                                {$nestedData[] = "<a href = 'CreateCustomer.php?id=$roomID&idb=$Book_Id&flag=$Flag '>เลือก</a>";}
                                        else 
                                        {
                                                $Flag = "Edit";
                                                $nestedData[] = "<a href = 'CreateCustomer.php?id=$roomID&idb=$Book_Id&flag=$Flag'>ยืนยันการเข้าพัก</a>"; 
                                        }
