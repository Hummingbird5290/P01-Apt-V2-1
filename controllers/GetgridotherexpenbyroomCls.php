	...
	$BiRoId = $id_roombill->Id;
		...
		$sql = "SELECT CONCAT((SELECT Bill_Detail FROM bill_groupdetail WHERE Bill_GroupDetail_No = 4),' ',
				...
				br.Id billId,bw.Id rowId,1 TypeTab
		...
		SELECT CONCAT((SELECT Bill_Detail FROM bill_groupdetail WHERE Bill_GroupDetail_No = 5),'  ',
				'(',be.Electricity_Unit,'-',be.Electricity_Unit_End,')' ,' ',
					...
					br.Id billId,be.Id rowId,2 TypeTab
		...
		FORMAT(bc.Roomclean_Lease,2) Price,br.Id billId,bc.Id rowId,3 TypeTab
		...
		FORMAT(bd.Damage_Lease,2) Price,br.Id billId ,bd.Id rowId,4 TypeTab
		...
		while( $row=mysqli_fetch_array($query) ) {  // preparing an array
				$nestedData=array(); 
						...
						if ($Price != NULL){
							...
							// $nestedData[] ="<a href ='CreateBill_Otherexpen.php?id=$Roomid&bid=$billId&row=$rowId&flag=Delete&TyTab=$typetap&sr=4' onclick=\"return confirm('ยืนยันลบรายการ!!!')\"><font color='red'>ลบ</font></a>";
						}						
				$data[] = $nestedData;
			}	
		$json_data = array(
