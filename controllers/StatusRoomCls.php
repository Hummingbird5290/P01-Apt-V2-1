...
$sql = "SELECT Room.Id,Room_No, RoomType_Id, room.Status_Room Status_Room, RoomType, RoomDetail, Room_Rates, rt.flag , 
        (CASE WHEN b.id IS null THEN CONCAT(cm.Title,' ',cm.Name, ' ',cm.Last_Name) 
        ELSE CONCAT(b.Title,' ',b.Name, ' ',b.Last_Name) END) As FullName,
        rs.RoomStatusDetail statusroom,room.Book_Id Book_Id,rs.Sort Sort,        
        (SELECT rst.RoomStatusDetail FROM bill_room br INNER JOIN roomstatus rst ON rst.RoomStatusId = br.Br_Status 
        WHERE  br.Room_Id = room.Id and 
        DATE_FORMAT(CAST(br.Create_Date	 as DATE),'%m/%Y') = DATE_FORMAT( CAST(NOW() as DATE), '%m/%Y') 
        ORDER BY br.Id DESC LIMIT 1) statusbill,
        (SELECT rst.Id FROM bill_room br INNER JOIN roomstatus rst ON rst.RoomStatusId = br.Br_Status 
        WHERE  br.Room_Id = room.Id 
        and DATE_FORMAT(CAST(br.Create_Date as DATE),'%m/%Y') = DATE_FORMAT( CAST(NOW() as DATE), '%m/%Y') 
        ORDER BY br.Id DESC LIMIT 1) statusId       
        FROM room 
        ...
        -- LEFT JOIN bill_room br on room.Id = br.Room_Id 
        -- INNER JOIN roomstatus rss ON br.Br_Status = rss.Id
        WHERE  
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
		$nestedData=array(); 
				...
				if($statusId==5 )
                                {
                                $nestedData[] = "<span class=\"label label-default\">" . $statusbill. "</span>";
                                }
                                elseif($statusId==6)
                                {
                                $nestedData[] = "<span class=\"label label-warning\">" . $statusbill. "</span>";
                                }
                                elseif($statusId == 7)
                                {
                                $nestedData[] = "<span class=\"label label-danger\">" . $statusbill. "</span>";
                                }elseif($statusId==8 )
                                {
                                $nestedData[] = "<span class=\"label label-warning\">" . $statusbill . "</span>";
                                }elseif($statusId==9 ) 
                                {
                                $nestedData[] = "<span class=\"label label-success\">" .  $statusbill  . "</span>";    
                                }else 
                                {
                                $nestedData[] = "<span class=\"label label-info\">" . $statusbill. "</span>";  
                                } 
