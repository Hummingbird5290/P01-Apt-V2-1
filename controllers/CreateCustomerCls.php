...
class Customer {
        ...
        {     
                $result = false;                 
                $sql="SELECT *  FROM customer where Id = $RoomId Order by Id Desc limit 1 ;";		
        ...
        {     
                $result = false;                 
                $sql="SELECT CONCAT(Title,' ',Name, ' ',Last_Name) As FullName  FROM customer where Id = $Id ORDER BY Id DESC LIMIT 1 ;";		
		$Maxs =  $this->db->query($sql) ;
                $count_row = $Maxs->num_rows;
                if (isset($count_row)){ 
                        $obj = mysqli_fetch_object($Maxs);
	...
	//insert Customer	       
        public function CreateCustomar($RoomId,$Title, $Name, $Lastname, $CardId, $Email, $Tel, $Tel2, $Address,$user,$Book_Amount,$DateCon,$Idb)
        {      
